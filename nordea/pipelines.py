...
class NordeaPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `nordea`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM nordea WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO nordea (date, title, link, content)"
